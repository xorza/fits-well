...
| §8 | WCS | Partial: image and table keyword translation, all 27 standard celestial projections, every analytic Table-26 spectral pair, and generic `LOG` are covered; `GRI`, `GRA`, and `TAB` remain explicitly unsupported. |
...
- [x] **Do not return `Ok(world)` from a transform whose nonlinear stage was not evaluated.** Unsupported-axis classification covers celestial projections, unimplemented spectral algorithms, and generic four-character `TAB` forms, including `TIME-TAB` (`docs/refs/fits_standard40.md:3779-3801`). Raw HDU/WCS parsing remains permissive, but `pixel_to_world` and `world_to_pixel` return `UnsupportedWcsTransform` whenever any required nonlinear stage is unavailable. XPH, `FREQ-TAB`, and `TIME-TAB` tests verify that partial coordinates cannot be returned as complete results.
...
- [ ] **Implement the remaining standard WCS nonlinear coordinate algorithms.** All 27 Table-23 celestial projections and the Table-26 analytic `F2*`/`W2*`/`V2*`/`A2*` plus generic `LOG` algorithms are evaluated with wcslib 8.5 cross-checks (`docs/refs/fits_standard40.md:3551-3598`, `:3779-3801`; `src/wcs/axis.rs`). The remaining standard gap is `GRI`, `GRA`, and `TAB`; XPH remains a separately reported convention. Until each coordinate algorithm lands, complete transforms report it unsupported as required by Batch 1.

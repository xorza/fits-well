...
| §8 | WCS | Partial: image and table keyword translation plus all 27 standard celestial projections are covered; unsupported transforms are explicitly rejected. The Table-26 nonlinear coordinate algorithms remain unevaluated. |
...
- [ ] **Implement the remaining standard WCS nonlinear coordinate algorithms.** All 27 Table-23 celestial projections are evaluated: `TSC`, `CSC`, `QSC`, and parameterized `HPX` now include face/facet domain checks and wcslib 8.5 cross-checks (`docs/refs/fits_standard40.md:3551-3598`). The remaining gap is the Table-26 spectral `F2*`/`W2*`/`V2*`/`A2*`, `LOG`, `GRI`, `GRA`, and `TAB` algorithms (`:3779-3801`). XPH remains a separately reported convention. Until each coordinate algorithm lands, complete transforms report it unsupported as required by Batch 1.
